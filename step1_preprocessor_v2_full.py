#!/usr/bin/env python3
"""
Impact Slide Preprocessor v2 (Full Merged Version)
====================================================
This is an improved version of step1_preprocessor.py with focus on
creating a much richer Evidence Register from Excel files.

Key Improvements (Excel + PPTX):
1. Identifier column detection (S.No, ID, Serial, etc.)
2. Significantly better findings from categorical/text columns
3. Evidence priority scoring (0.0 - 1.0)
4. Basic multi-column insights
5. Rich PPTX slide classification & evidence extraction (charts, tables, metrics, bullets)

Usage:
    python step1_preprocessor_v2_full.py --input /path/to/source_files --output ./output
"""
...
class ImpactSlidePreprocessorV2:
    ...
    def build_evidence_register(self) -> List[Dict]:
        ...
        # === Item 4.2: Evidence Deduplication ===
    ...
    def _deduplicate_evidence(self, evidence: List[Dict]) -> List[Dict]:
        """
        Item 4.2: Remove near-duplicate evidence entries.
        ...
        for ev in evidence:
            ...
            norm_text = re.sub(r'\s+', ' ', text)[:120]   # first 120 chars normalized
    ...
    def _save_outputs(self):
        ...
        if self.export_csv:
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=["evidence_id", "insight_type", "text", "priority_score", "confidence", "suggested_narrative_use", "source_location"])
                writer.writeheader()
                writer.writerows(self.evidence_register)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Impact Slide Preprocessor v2")
