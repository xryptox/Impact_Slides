...
def test_strict_rejects_q4_s05_one_point_line_shape():
    """Q4 2021 s05 leftover (#296): 8 quarters, 3 series, only Q4'21 finite."""
    raw = _raw()
    ...
    vis["chart_data"]["series"] = [
        {"series_id": "online", "name": "Online", "values": [None] * 7 + ["31"]},
        {"series_id": "offline", "name": "Offline", "values": [None] * 7 + ["12"]},
        {"series_id": "total", "name": "Total G&S", "values": [None] * 7 + ["24"]},
    ]
    ...
    assert "online" in contracts

    # Second finite per series is legal; s05 must not invent that point.
    for series in vis["chart_data"]["series"]:
        series["values"][6] = "1"
    assert validate_handoff(raw, strict=True).ok


def test_strict_rejects_q4_s08_five_series_line_shape():
    """Q4 2021 s08 leftover (#297): 5 T&E series, unlabeled vs-2019 interiors."""
    raw = _raw()
    vis = raw["slides"][1]["payload"]["chart"]
    vis["chart_data"]["categories"] = [
        {"category_id": cid, "label": lab}
    ...
    # Page-8 % of Q4'19 callouts are not vs-2019 plot glyphs.
    vis["chart_data"]["series"] = [
        {"series_id": "us-cons", "name": "US Consumer", "values": [None] * 7 + ["108"]},
        {"series_id": "sme", "name": "Total SME", "values": [None] * 7 + ["83"]},
        {"series_id": "total-te", "name": "Total T&E", "values": [None] * 7 + ["82"]},
        {"series_id": "intl-cons", "name": "Intl Consumer", "values": [None] * 7 + ["78"]},
        {"series_id": "lg", "name": "Large & Global Corporate", "values": [None] * 7 + ["36"]},
    ]
    with pytest.raises(RendererValidationError) as ei:
        validate_handoff(raw, strict=True)
    contracts = " ".join(
    ...
    # Dropping the 5th series still fails: one finite is not a vs-2019 pair.
    ...
    # Second finite on 4 series is legal; s08 must not invent that point.
    for series in vis["chart_data"]["series"]:
        series["values"][6] = "1"
    assert validate_handoff(raw, strict=True).ok
