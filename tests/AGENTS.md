...
- Renderer v3 kernel (#175/#197): `test_renderer_v3_kernel.py` + fixture `renderer_v3/minimal_cover_narrative_cover.json` (typed Deck validation, D252–D254 chart/charts/hero/support slots, leftover v2 aliases rejected, D170/D253/D254 dual/hero heading requirement + non-strict Untitled wording, allowlisted repairs, schema drift, v2 isolation + mutation traps)
...
- Renderer v3 chart support surfaces (#189/#197): `test_renderer_v3_chart_support.py` (typed `support_table`/`outlined_support`/`metric_strip` on `single_chart` and `chart_hero_dual`; shared D252 category/alignment + hero support_table centers; non-strict drops unknown hero support fields on every slide; outlined centers ≤2px; D47 320×240 plot floor; complete rows/metrics; mutation traps)
