...
- Renderer v3 chart facts (#188): `test_renderer_v3_chart_facts.py` + fixtures `renderer_v3/minimal_line_chart.json` / `minimal_heatmap.json` (structured context/annotation/measurement chrome; one-block short_label then below-plot relocate; relocated block must fit existing pad without growing `view_h`; heatmap chrome-height matches painted fact rows)
