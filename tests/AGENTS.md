...
- Renderer v3 annex/comparison tables (#180/#231): `test_renderer_v3_annex_comparison.py` + fixture `renderer_v3/annex_and_comparison_tables.json` (`annex_table`/`grouped_annex_table`/`period_comparison`/`comparison_cards`; grouped headers + disclosure; fixed period roles + metric strip; peer cards; circular dual-metric recipe for 2-number+`Nx` boards; non-strict complete-data fallbacks; pre-validation table cell/group repair; cards a11y/print contract; peer-heading fit threshold; sync floor bound; mutation traps)
- Renderer v3 line chart (#182/#224): `test_renderer_v3_line_chart.py` + fixture `renderer_v3/minimal_line_chart.json` (typed single_chart line envelope, null gaps, frozen plan, Chart.js + noscript SVG parity, one semantic table, identity/point labels, no gridlines, readiness payloads, DP-1 tick/value floors + semibold weight, mutation traps)
...
- Renderer v3 combo charts (#185): `test_renderer_v3_combo_chart.py` + fixture `renderer_v3/minimal_combo_chart.json` (grouped/stacked `bar_mode`, bar+line layers, primary/secondary axis ownership, complete identity, Chart.js/SVG layer order, semantic table, mutation traps)
...
- Renderer v3 chart support surfaces (#189/#197): `test_renderer_v3_chart_support.py` (typed `support_table`/`outlined_support`/`metric_strip` on `single_chart` and `chart_hero_dual`; shared D252 category/alignment + hero support_table centers painted via `_paint_chart_support`; non-strict drops unknown hero support fields on every slide; outlined centers ≤2px; D47 320×240 plot floor; complete rows/metrics; mutation traps)
- Renderer v3 canonical Amex corpus (#196/#226 / D54/D126/D314 / #224): `test_renderer_v3_canonical_amex.py` + fixture `renderer_v3/canonical_amex_handoff_v1.json` (44-slide D314 compositions/sections/evidence; s4/s5/s9/s10/s19 callouts + support tables; slide 6 ~6pp claim; slide 21 Capital Summary; no operational speaker notes; strict-clean Chart.js + SVG-only render; run_meta chart surfaces freeze ticks ≥20 / values ≥18; rebuild via `scripts/build_canonical_amex_v1.py`; locators pin the D315 PDF SHA-256)
- Renderer v3 3.0.0 release evidence (#198 / D315): `test_renderer_v3_release_evidence.py` + `artifacts/renderer_3_release/3.0.0/` (hash-pinned snapshot inputs, exact-five D250 roots, required gates, approved D55 divergences, no whole-slide scoring; live corpus may advance after 3.0.0; PDF SHA still matches; verify via `scripts/renderer_3_release.py --verify`)
