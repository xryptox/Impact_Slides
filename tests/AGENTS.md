...
- Renderer v3 annex/comparison tables (#180): `test_renderer_v3_annex_comparison.py` + fixture `renderer_v3/annex_and_comparison_tables.json` (`annex_table`/`grouped_annex_table`/`period_comparison`/`comparison_cards`; grouped headers + disclosure; fixed period roles + metric strip; peer cards; non-strict complete-data fallbacks; mutation traps)
