...
| hero / `metric_overview` heading / body / value | 22 / 16 / 44 | **32 / 27 / 72** (s12/s18/s21/s22; #273 target 28, landed 27 after s18 overflow; strip stays 44) | 700 on values |
...
- `#249`/`#256` extensions (slide maps + floors live next to the helpers): `measured_stub_ratio` (`DESIGN_LEDGER_STUB_RATIO_SLIDES` s3/s16/s31–37, stub ≤45%), `measured_support_chrome` (`DESIGN_LEDGER_SUPPORT_CHROME_SLIDES` empty after #269; remaining category-aligned hide_header fixtures still accept hairline body cells; a missing body frame still fails; painted `.head` still asserts navy band + hairlines), `measured_series_palette` (s24/s28; no non-semantic `#0A7D55`; sky_blue where authored), `measured_metric_value_styles` (s8/s12 ≥40px), `measured_bar_occupancy` (s28 ≥0.5)
...
| 15 | shipped #227 / #260 / #272 | reserve-rate row + authored totals + `stack_segments: show` + reserve `navy` + uncollided stack totals | `tests/test_amex_s6_s8_s15_furniture.py` |
