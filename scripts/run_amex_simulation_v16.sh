#!/usr/bin/env bash
# GNHF v16: 44-page PDF vs renderer_v3 SBS with DP-6 design ledger,
# validating the post-v15 ticket set (#268 s12 PDF hero KPI sentences,
# #269 s4/s19 independent navy-header tables, #270 legal unmarked
# continuations as <p>, #271 s18 folded Volume/Margin labels, #272 s15
# reserve navy + uncollide stack totals, #273 hero KPI 27px + wrap,
# #274 independent support-table thead stub navy).
set -euo pipefail
...
V16_HOME="${LOCALAPPDATA:-C:/Users/Ag1Le/AppData/Local}/Temp/gnhf-v16-supergrok"
PI_CLI="C:/Users/Ag1Le/AppData/Roaming/npm/node_modules/@earendil-works/pi-coding-agent/dist/cli.js"
V15_REPORT_COMMIT="0f67fb3"
...
git cat-file -e "$V15_REPORT_COMMIT" 2>/dev/null || {
  echo "Missing v15 report commit: $V15_REPORT_COMMIT" >&2; exit 1;
}

rm -rf "$V16_HOME"
mkdir -p "$V16_HOME/.gnhf"
PI_WRAPPER="$V16_HOME/pi-supergrok.cmd"
{
  printf '@ECHO off\r\n'
...
cat > "$V16_HOME/.gnhf/config.yml" <<EOF
agent: pi
agentPathOverride:
...
if ! (cd "$V16_HOME" && USERPROFILE="$V16_HOME" HOME="$V16_HOME" \
...
Objective: Produce the v16 Amex Q1 2026 observation baseline: a complete
...
re-validates the deck after the post-v15 ticket set: #268 s12 PDF hero KPI
sentences, #269 s4/s19 independent navy-header support tables, #270
legal_notice unmarked continuations as <p>, #271 s18 folded Volume/Margin
driver labels, #272 s15 reserve navy + uncollide stack totals, #273 hero
KPI label 27px + wrap, #274 independent support-table thead stub navy.
This is a Companion-mode SIMULATION / OBSERVATION run, not implementation.
Do not implement fixes or file issues.
...
authoring corrections and the #254–#274 corpus updates — do NOT run
amex_handoff_mutations.py on it):
  tests/fixtures/renderer_v3/canonical_amex_handoff_v1.json

Prior evidence:
  v15 report (never merged to main; read via:
    git show 0f67fb3:wiki/baseline_v15_GAP_ANALYSIS.md)
...
- One new tracked report: wiki/baseline_v16_GAP_ANALYSIS.md, copied verbatim
...
- Read-only use of source PDF, canonical corpus fixture, current tests,
  scripts/simulation_probe.py, the v15 report via git show, and issue/PR
  history.
...
Also run the #249/#256/#269 DP-6 extension probes (import the DESIGN_LEDGER_*
maps and helpers from simulation_probe.py; zero matches / ProbeError =
failure):
- measured_stub_ratio on DESIGN_LEDGER_STUB_RATIO_SLIDES (s3/s16/s31-37;
  stub share <= 0.45) — held from #246
- measured_support_chrome on DESIGN_LEDGER_SUPPORT_CHROME_SLIDES (empty
  tuple; Amex s4/s19 are independent navy-header tables per #269 — do NOT
  expect hide_header / .support-cat-cell.head chrome on those slides;
  missing body frame still fails on any remaining category-aligned
  hide_header fixture the helper is pointed at)
...
Record one design-ledger object per slide in comparison_manifest.json:
  ...
  furniture rows (selector, expected_text, count, ok), #249/#256/#269
  extension rows present for the slides above (stub_ratio / support_chrome /
  palette / metric_floor / bar_occupancy), and overall ok.
...
Read the v15 report with `git show 0f67fb3:wiki/baseline_v15_GAP_ANALYSIS.md`
...
divergence, citing the v16 SBS and design-ledger evidence. Explicitly cover
the post-v15 ticket surfaces:
- #268 s12: hero KPI labels are the PDF sentences
  "Global Consumer New Accounts Acquired from Millennial / Gen-Z" (66%)
  and "Global New Accounts Acquired on Fee-Paying Products*" (73%); short
  share labels gone.
- #269 s4/s19: independent navy-header support tables (band-table-header
  period columns visible); not category-aligned hide_header. v15 A-247
  hide-header acceptance on these two slides should flip.
- #270 s38–43: unmarked-only legal bodies paint as <p> (s39–43: N
  paragraphs, zero <ul>/<li>); s38 stays preamble <p> + two marked <ul>s.
  v15 R-D packing remainder should flip or shrink to leftover air/type.
- #271 s18: Volume/Margin folded into one driver label
  ("Volume - Total Balances", "Margin - Net Interest Income / Average
  Total Balances"); no leftover driver-detail spans.
- #272 s15: Reserve Build/(Release) is navy (not sky_blue); write-offs
  stay primary_blue; stack totals do not collide with Q2'25–Q4'25
  segment labels. Do not invent a third series.
- #273 hero/`metric_overview` body is 27px with wrapping KPI labels
  (HERO_HEADING_PX 32 / HERO_VALUE_PX 72 / METRIC_STRIP_VALUE_PX 44 held).
- #274 s9/s10 (and s4/s19 after #269): independent support-table thead
  stub (Q1'26 / Metric) keeps the navy band; tbody stubs stay transparent.
- any v15 residual outside those tickets (covers, period pills,
  comparison cards, metric_overview, section divider, annex flatten,
  s24 navy Processed Volumes — parked / accepted unless fresh evidence
  changes the class).
...
wiki/baseline_v16_GAP_ANALYSIS.md. Include renderer commit and version,
...
ledger, v15->v16 delta, residual triage, diagnostics, and artifact links.

Create exactly two commits in order:
1. `sim: store v16 renderer_v3 Amex full comparison artifacts`
   Force-add only simulation/amex_q1_2026/.
2. `docs: store v16 renderer_v3 complete-deck comparison`
   Add only wiki/baseline_v16_GAP_ANALYSIS.md.
...
USERPROFILE="$V16_HOME" HOME="$V16_HOME" \
gnhf \
  --agent pi \
  --max-iterations 10 \
  --max-tokens 10000000 \
  ...
  --push \
  --prevent-sleep on \
  --stop-when "A v16 renderer_v3 observation baseline with all 44 PDF/HTML side-by-sides, a qualitative ledger, a DP-6 design ledger, v15->v16 delta covering #268-#274, residual triage, and exactly two allowed commits exists; no production paths changed and no image scoring was used." \
  "$PROMPT"
