...
V14_HOME="${LOCALAPPDATA:-C:/Users/Ag1Le/AppData/Local}/Temp/gnhf-v14-supergrok"
PI_CLI="C:/Users/Ag1Le/AppData/Roaming/npm/node_modules/@earendil-works/pi-coding-agent/dist/cli.js"
V13_REPORT_COMMIT="ea541bf"
...
git cat-file -e "$V13_REPORT_COMMIT" 2>/dev/null || {
  echo "Missing v13 report commit: $V13_REPORT_COMMIT" >&2; exit 1;
}

rm -rf "$V14_HOME"
mkdir -p "$V14_HOME/.gnhf"
PI_WRAPPER="$V14_HOME/pi-supergrok.cmd"
{
  printf '@ECHO off\r\n'
...
cat > "$V14_HOME/.gnhf/config.yml" <<EOF
agent: pi
agentPathOverride:
...
if ! (cd "$V14_HOME" && USERPROFILE="$V14_HOME" HOME="$V14_HOME" \
...
Objective: Produce the v14 Amex Q1 2026 observation baseline: a complete
...
re-validates the deck after the post-v13 renderer fixes: #246 table
stub-slack cap + sparse bar occupancy, #247 category-aligned support-table
chrome, #248 sky-blue palette activation + green-cycle removal + KPI metric
floor + line-label contrast freeze. This is a Companion-mode SIMULATION /
...
authoring corrections and the #248 corpus updates — do NOT run
amex_handoff_mutations.py on it):
  tests/fixtures/renderer_v3/canonical_amex_handoff_v1.json

Prior evidence:
  v13 report (never merged to main; read via:
    git show ea541bf:wiki/baseline_v13_GAP_ANALYSIS.md)
...
- One new tracked report: wiki/baseline_v14_GAP_ANALYSIS.md, copied verbatim
...
- Read-only use of source PDF, canonical corpus fixture, current tests,
  scripts/simulation_probe.py, the v13 report via git show, and issue/PR
  history.
...
- measured_metric_value_styles on DESIGN_LEDGER_METRIC_FLOOR_SLIDES (s8/s12;
  value font-size >= 40px) — validates #248 KPI floor
- measured_bar_occupancy on DESIGN_LEDGER_BAR_OCCUPANCY_SLIDES (s28;
  bar width / category pitch >= 0.5) — validates #246 occupancy
...
Read the v13 report with `git show ea541bf:wiki/baseline_v13_GAP_ANALYSIS.md`
...
- the #248 surfaces: sky-blue series activation and green-cycle removal on
  ...
  white;
- any v13 residual outside those tickets (typography, furniture, corpus).
...
wiki/baseline_v14_GAP_ANALYSIS.md. Include renderer commit and version,
...
ledger, v13->v14 delta, residual triage, diagnostics, and artifact links.

Create exactly two commits in order:
1. `sim: store v14 renderer_v3 Amex full comparison artifacts`
   Force-add only simulation/amex_q1_2026/.
2. `docs: store v14 renderer_v3 complete-deck comparison`
   Add only wiki/baseline_v14_GAP_ANALYSIS.md.
...
USERPROFILE="$V14_HOME" HOME="$V14_HOME" \
gnhf \
  --agent pi \
  --max-iterations 10 \
  --max-tokens 10000000 \
  ...
  --push \
  --prevent-sleep on \
  --stop-when "A v14 renderer_v3 observation baseline with all 44 PDF/HTML side-by-sides, a qualitative ledger, a DP-6 design ledger, v13->v14 delta, residual triage, and exactly two allowed commits exists; no production paths changed and no image scoring was used." \
  "$PROMPT"
