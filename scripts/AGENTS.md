...
- `amex_handoff_mutations.py` — bounded Amex handoff authoring fixes applied before a fidelity pass (issue #148 slides 13–14 bars/pane order); does not change renderer defaults. CLI: `python scripts/amex_handoff_mutations.py IN.json -o OUT.json`
...
- Next Amex fidelity pass must run `amex_handoff_mutations.apply_issue_148_bar_semantics` (or the CLI) on the copied baseline handoff before render so slides 13–14 keep grouped/dual vertical bars and PDF pane order; preserve the mutated handoff under the simulation pass as evidence. Contract: `tests/test_amex_s13_s14_handoff_contract.py`.
