#!/usr/bin/env bash
# GNHF v13: 44-page PDF vs renderer_v3 SBS with DP-6 design ledger.
set -euo pipefail
...
V13_HOME="${LOCALAPPDATA:-C:/Users/Ag1Le/AppData/Local}/Temp/gnhf-v13-supergrok"
...
rm -rf "$V13_HOME"
mkdir -p "$V13_HOME/.gnhf"
PI_WRAPPER="$V13_HOME/pi-supergrok.cmd"
{
  printf '@ECHO off\r\n'
...
cat > "$V13_HOME/.gnhf/config.yml" <<EOF
agent: pi
agentPathOverride:
...
if ! (cd "$V13_HOME" && USERPROFILE="$V13_HOME" HOME="$V13_HOME" \
...
Objective: Produce the v13 Amex Q1 2026 observation baseline: a complete
...
both the qualitative content ledger and the DP-6 design ledger. This is a
Companion-mode SIMULATION / OBSERVATION run, not implementation. Do not
implement fixes or file issues.
...
Prior evidence:
  wiki/baseline_v12_GAP_ANALYSIS.md (v3 first baseline)
  wiki/SPEC_renderer_v3_pdf_design_parity.md (DP-1..DP-6)
...
- One new tracked report: wiki/baseline_v13_GAP_ANALYSIS.md, copied verbatim
...
For every residual named in wiki/baseline_v12_GAP_ANALYSIS.md, state whether
the current render resolves it, preserves it, or replaces it with a different
divergence, citing the v13 SBS and design-ledger evidence. Explicitly cover
the C2 typography collapse (14px/400 vs 20px/600) and the C1 furniture
backfills (slides 4/5/6/8/9/10/12/15/17/18/19/21/24/28). Add a short section
on what renderer_v3 now does well.
...
wiki/baseline_v13_GAP_ANALYSIS.md. Include renderer commit and version,
...
ledger, v12->v13 delta, residual triage, diagnostics, and artifact links.

Create exactly two commits in order:
1. `sim: store v13 renderer_v3 Amex full comparison artifacts`
   Force-add only simulation/amex_q1_2026/.
2. `docs: store v13 renderer_v3 complete-deck comparison`
   Add only wiki/baseline_v13_GAP_ANALYSIS.md.
...
USERPROFILE="$V13_HOME" HOME="$V13_HOME" \
gnhf \
  --agent pi \
  --max-iterations 10 \
  --max-tokens 10000000 \
  ...
  --push \
  --prevent-sleep on \
  --stop-when "A v13 renderer_v3 observation baseline with all 44 PDF/HTML side-by-sides, a qualitative ledger, a DP-6 design ledger, v12->v13 delta, residual triage, and exactly two allowed commits exists; no production paths changed and no image scoring was used." \
  "$PROMPT"
