#!/usr/bin/env bash
# GNHF v12: first full 44-page PDF vs renderer_v3 (schema-v1) side-by-side baseline.
set -euo pipefail
...
V12_HOME="${LOCALAPPDATA:-C:/Users/Ag1Le/AppData/Local}/Temp/gnhf-v12-supergrok"
...
rm -rf "$V12_HOME"
mkdir -p "$V12_HOME/.gnhf"
PI_WRAPPER="$V12_HOME/pi-supergrok.cmd"
{
  printf '@ECHO off\r\n'
...
cat > "$V12_HOME/.gnhf/config.yml" <<EOF
agent: pi
agentPathOverride:
...
if ! (cd "$V12_HOME" && USERPROFILE="$V12_HOME" HOME="$V12_HOME" \
...
Objective: Produce the v12 Amex Q1 2026 fidelity baseline: the first complete
44-page side-by-side comparison of the source PDF deck against HTML slides
rendered by the new renderer_v3 (schema-v1 kernel, 3.0.0) from the canonical
D314 corpus. This is a Companion-mode SIMULATION / OBSERVATION run, not
implementation. Determine what renderer_v3 demonstrably reproduces across the
complete deck and what still diverges.
...
If wiki/AGENT_LEARNINGS.md exists, read it and apply its capture lessons.
...
authoring corrections — do NOT run amex_handoff_mutations.py on it):
  tests/fixtures/renderer_v3/canonical_amex_handoff_v1.json

Prior evidence:
  wiki/baseline_v11_GAP_ANALYSIS.md (renderer_v2 baseline, when present)
  artifacts/renderer_3_release/3.0.0/ (D315 acceptance bundle)

=== STRICT SCOPE ===

Allowed:
- New files only below simulation/amex_q1_2026/.
- One new tracked report: wiki/baseline_v12_GAP_ANALYSIS.md, copied verbatim
...
- Read-only use of source PDF, canonical corpus fixture, current tests,
  scripts/simulation_probe.py, prior baselines, and issue/PR history.
...
- Claims based only on PR titles, old screenshots, fixed sleeps, ordinal slide
  selectors, zero-match probes, or unpainted Chart.js geometry.
...
- Do NOT call painted_datalabel_lines on the v3 deck: it targets the
  ...
  its own context_labels/annotations/measurements chrome. Where label content
  evidence is needed, read the painted v3 DOM/canvas instead.
...
Screenshot all 44 HTML slides at exactly 1920x1080 after the identity and
paint-readiness checks. Create full-resolution side-by-sides (3840x1080,
PDF left / renderer_v3 right) without downscaling either half, a labeled
44-slide contact sheet, and comparison_manifest.json proving every PDF/HTML/
SBS artifact exists with its slide identity.
...
qualitatively; do not score images.

=== V2 -> V3 DELTA ===

For every renderer residual named in wiki/baseline_v11_GAP_ANALYSIS.md (when
available), state whether the renderer_v3 render resolves it, preserves it,
or replaces it with a different divergence, citing the v12 SBS evidence. Add a
short section on what renderer_v3 now does well.
...
wiki/baseline_v12_GAP_ANALYSIS.md. Include renderer commit and version,
source identities, render outcome (clean or DEGRADED with diagnostics), scope
audit, mapping assertion, capture contract, 44-row ledger, v2->v3 delta,
residual triage, diagnostics, and artifact links.

Create exactly two commits in order:
1. `sim: store v12 renderer_v3 Amex full comparison artifacts`
   Force-add only simulation/amex_q1_2026/.
2. `docs: store v12 renderer_v3 complete-deck comparison`
   Add only wiki/baseline_v12_GAP_ANALYSIS.md.
...
USERPROFILE="$V12_HOME" HOME="$V12_HOME" \
gnhf \
  --agent pi \
  --max-iterations 10 \
  --max-tokens 10000000 \
  ...
  --push \
  --prevent-sleep on \
  --stop-when "A v12 renderer_v3 observation baseline with all 44 PDF/HTML side-by-sides, a qualitative ledger, v2->v3 delta, residual triage, and exactly two allowed commits exists; no production paths changed and no image scoring was used." \
  "$PROMPT"
