...
Schema-v1 canonical rendering kernel for Impact Slide Renderer 3. Sibling of frozen legacy `renderer_v2`. Owns typed validation into one canonical deck model, deck-wide measure/plan freeze for kernel prose/chrome/tables/axis charts/heatmaps/linear+grouping compositions, decimal-safe number formatting, deterministic five-artifact publication, the sole boardroom_amex theme manifest (CSS/Chart.js/SVG tokens), axis-chart painters for line + grouped/horizontal bars (Chart.js + noscript SVG + D247 semantic table), native semantic heatmaps (D163/D246/D308), process/timeline/layered/pipeline compositions, and the offline legacy→v1 migration tool (D119/D313); remaining chart and relationship families land in later tickets.

## Ownership

- Closed schema-v1 handoff models (`models.py`) including D213 semantic values + D255 `table` + table compositions (`data_table`, `annex_table`, `grouped_annex_table`, `period_comparison`, `comparison_cards`) + linear/grouping compositions (`process_flow`, `timeline`, `layered_architecture`, `data_pipeline`) + `single_chart` envelope for `line` / `grouped_bar` / `horizontal_bar` / `heatmap` (D163/D227–D240/D243/D246/D272–D277/D290–D302/D308) with optional `category_groups` + boxed-label `auxiliary_series`
- Decimal-safe format registry (`format.py`) for D77/D78/D103/D214/D293
- Axis-chart freeze + dual painters + semantic table for line and grouped/horizontal bars; heatmap freeze + native HTML painter (`charts.py`) (D53/D57/D63/D71–D73/D106/D157/D160/D163/D246–D248/D302/D308)
...
- Kernel compositions: `opening_cover`, `section_divider`, `closing_cover`, `narrative`, `legal_notice`, `data_table`, `annex_table`, `grouped_annex_table`, `period_comparison`, `comparison_cards`, `process_flow`, `timeline`, `layered_architecture`, `data_pipeline`, `single_chart` (`line` / `grouped_bar` / `horizontal_bar` / `heatmap`) (D178–D187/D192–D193/D196–D197/D208/D210/D215/D223/D226/D251/D257–D261/D268–D273/D276–D277/D287/D302/D308)
