...
- Deck-wide adaptive measure/plan freeze (`plan.py`) for narrative prose, data tables, axis charts (incl. stacked bars) + chart support surfaces, heatmaps, linear/grouping + relationship/decision + cards/reviews + dual/hero/metric compositions (fixed D60 type where specified; feature_cards adaptive 22–28), with fit heights matching publish CSS gaps/margins/paddings (`LIST_INDENT_EM`, `TRANSITION_STEP_PAD`, card panel chrome), subtitles, takeaways, disclosures, source footers, and fixed chrome (D1–D4, D10/D47, D22–D25, D44, D59, D60, D68–D70, D201–D206/D281–D286)
...
- `single_chart` axis charts: one frozen `chart_paint` drives Chart.js canvas + noscript SVG + one D247 semantic table; line nulls break paths; grouped/horizontal bars preserve signed/zero/null slots with outside values, optional D237 groups + D235 boxed labels, and D157 leading break (horizontal_bar additionally enforces the D243 positive-side contract); `stacked_bar` is sign-separated vertical stacks (2–6 series, 1–12 cats), forbids ordinary_values + pane_title identity, optional one `authored_stack_total` auxiliary, domain must cover zero + signed stack extents (D242/D295/D304); no gridlines; transparent plot/body; readiness flags `semantic_table_present` + `chart_painters`
- `single_chart` forbids `support_visual` until category-aligned support lands (deliberate D140 deferral); hero support stays on `chart_hero_dual` only
...
- `chart_hero_dual`: left axis chart + optional left support (`support_table` / `outlined_support` / `metric_strip`) + right hero (`metric_stack` 1–3 or `driver_card` 1–6); plan stacks left column vs hero and applies frozen hero `role_sizes` at paint (D150–D153)
