...
Schema-v1 canonical rendering kernel for Impact Slide Renderer 3. Sibling of frozen legacy `renderer_v2`. Owns typed validation into one canonical deck model, deck-wide measure/plan freeze for kernel prose/chrome/tables, decimal-safe number formatting, deterministic five-artifact publication, and the sole boardroom_amex theme manifest (CSS/Chart.js/SVG tokens); full chart painting lands in later tickets.
...
- Brand slides: covers share one `CoverPayload` (title + optional subtitle/period/date); divider payload is only `section_id` with registry-derived label + section ordinal; renderer owns bands/rules/chrome; no root title/section/takeaway/disclosure/source_footer
