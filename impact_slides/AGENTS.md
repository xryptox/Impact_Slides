...
- `renderer_v3/AGENTS.md` — schema-v1 canonical kernel (typed validation, repairs, deck-wide plan freeze, table + linear/grouping + relationship/decision compositions + format registry, line-chart tracer, grouped/horizontal/stacked bar charts, explicit arithmetic waterfalls, chart support surfaces, native heatmaps, offline legacy→v1 migrator, generated JSON Schema, deterministic five-artifact `render_deck` publication); remaining chart/card families arrive in later tickets; cards/reviews (#194/#215)
