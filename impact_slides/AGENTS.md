...
- `renderer_v3/AGENTS.md` — schema-v1 canonical kernel (typed validation, repairs, deck-wide plan freeze, table + linear/grouping + relationship/decision + cards/reviews + dual/hero/metric + chart_grouped_annex compositions + format registry, line-chart tracer, grouped/horizontal/stacked bar charts, typed grouped/stacked combo charts, explicit arithmetic waterfalls, pie/donut mix charts, chart support surfaces, native heatmaps, structured context/annotation/measurement facts + collision-owned chrome, offline legacy→v1 migrator, generated JSON Schema, deterministic five-artifact `render_deck` publication); D314 canonical Amex corpus; D315 3.0.0 release evidence lives under `artifacts/renderer_3_release/`
