...
- `renderer_v3/AGENTS.md` — schema-v1 canonical kernel (typed validation, repairs, deck-wide plan freeze, table + linear/grouping + relationship/decision + cards/reviews + dual/hero/metric compositions + format registry, line-chart tracer, grouped/horizontal/stacked bar charts, typed grouped/stacked combo charts, explicit arithmetic waterfalls, chart support surfaces, native heatmaps, offline legacy→v1 migrator, generated JSON Schema, deterministic five-artifact `render_deck` publication); D314 canonical Amex corpus
