...
# #151: category-aligned boxed bar labels (semantic furniture, not datalabels).
_BOXED_LABELS_PLUGIN_HTML = """<script>(function(){if(typeof Chart==='undefined'||Chart.__rv2BoxedLabels)return;Chart.__rv2BoxedLabels=1;Chart.register({id:'rv2BoxedLabels',afterDatasetsDraw:function(chart){var opts=chart.config.options.plugins&&chart.config.options.plugins.boxedLabels;if(!opts||!opts.values||!opts.values.length)return;if(chart.options.indexAxis==='y')return;var meta=chart.getDatasetMeta(0);if(!meta||!meta.data)return;var ctx=chart.ctx,ys=chart.scales.y,area=chart.chartArea;if(!ys||!area)return;var fs=Math.max(11,Number(opts.minFontSize)||12),padX=6,padY=3,minBar=28,painted=0,outside=0;ctx.save();ctx.font='700 '+fs+"px 'IBM Plex Sans', sans-serif";ctx.textAlign='center';ctx.textBaseline='middle';try{opts.values.forEach(function(text,i){if(text==null||text==='')return;var el=meta.data[i];if(!el||typeof el.x!=='number')return;var x=el.x,base=typeof el.base==='number'?el.base:ys.getPixelForValue(0),top=el.y,bh=Math.abs(base-top),bw=Math.max(4,(el.width||20)),tw=ctx.measureText(String(text)).width,boxW=tw+padX*2,boxH=fs+padY*2,inside=bh>=Math.max(minBar,boxH+8)&&bw>=boxW+4,cx=x,boxY,ty;if(inside){boxY=top+bh*0.4-boxH/2;ty=boxY+boxH/2}else{outside++;var gap=10;if(top<=base){boxY=top-gap-boxH;ctx.beginPath();ctx.strokeStyle='#00175a';ctx.lineWidth=1;ctx.moveTo(cx,top);ctx.lineTo(cx,boxY+boxH);ctx.stroke()}else{boxY=top+gap;ctx.beginPath();ctx.strokeStyle='#00175a';ctx.lineWidth=1;ctx.moveTo(cx,top);ctx.lineTo(cx,boxY);ctx.stroke()}ty=boxY+boxH/2}var boxX=cx-boxW/2;ctx.fillStyle='#eef0f0';ctx.strokeStyle='#00175a';ctx.lineWidth=1;if(ctx.roundRect){ctx.beginPath();ctx.roundRect(boxX,boxY,boxW,boxH,3);ctx.fill();ctx.stroke()}else{ctx.fillRect(boxX,boxY,boxW,boxH);ctx.strokeRect(boxX,boxY,boxW,boxH)}ctx.fillStyle='#00175a';ctx.fillText(String(text),cx,ty);painted++})}finally{ctx.restore()}chart.canvas.dataset.rv2BoxedLabelsPainted=String(painted);chart.canvas.dataset.rv2BoxedLabelsOutside=String(outside);chart.canvas.dataset.rv2BoxedLabels='1';if(outside>0){chart.canvas.setAttribute('data-boxed-outside',String(outside));chart.canvas.setAttribute('aria-description','boxed labels: '+painted+' painted, '+outside+' outside bars');}}})})()</script>"""
