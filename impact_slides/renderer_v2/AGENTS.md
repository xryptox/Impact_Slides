...
- `chart_config.bar_groups` on vertical grouped bars is opt-in: Chart.js receives `options.plugins.barGroups.items` and the shell plugin paints labeled category-span brackets; absent/invalid groups leave existing Chart.js and SVG output unchanged. SVG continues to use its native brackets (#154)
