...
- Parallel ticket workflows must launch full interactive Pi TUI sessions through the durable Herdr scripts in `~/.pi/agent/pi-extensible-workflows/scripts/`, using visible tabs in the supervising session's current workspace; do not create a separate Herdr workspace, launch manually, use RPC, or use `pi --print`. Every wave must start the watcher that surfaces idle/done/blocked output, especially no-mistakes `ask-user` gates, back to the host session. Close each temporary tab only after collecting its implementation and no-mistakes report.
