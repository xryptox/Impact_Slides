...
- Parallel ticket workflows must launch full interactive Pi TUI sessions through the durable Herdr scripts in `~/.pi/agent/pi-extensible-workflows/scripts/`, using visible tabs in the supervising session's current workspace; do not create a separate Herdr workspace, launch manually, use RPC, or use `pi --print`. Every wave must start exactly one manifest-scoped deduplicating return-on-first-event watcher that surfaces each idle/done/blocked state signature once as a Herdr notification, especially no-mistakes `ask-user` gates; decision-relevant events are additionally forwarded to the supervising Pi session as completed-workflow follow-ups, while informational events (working/idle heartbeats, auto-fix gate parks) stay notification-only to save supervisor input tokens. Only unchanged `ask-user` decisions repeat under the two-notification pause policy. The watcher also emits `gate-parked` for parked auto-fix gates and `pr-ready` for the exact-head merge-approval handoff, and keeps supervising through a placeholder pane when an implementer pane disappears; pane loss never drops gate supervision. The watcher must also hard-stop and escalate no-mistakes loops after more than 5 review-fix rounds, more than 90 minutes in review fixing, or more than 10 minutes without agent/log activity; preserve pipeline custody and require controlled recovery rather than approving another fix. After all run-owned PRs are merged, linked issues are closed, implementers are terminal, and final reports are collectible, the watcher closes the temporary tabs and runs verified wave cleanup automatically; a merely closed/unmerged PR blocks cleanup. Watcher follow-up events are token-trimmed before delivery and only decision-relevant events reach the session; mechanics live in `docs/agents/delegated-delivery.md` (Follow-up token economy). Consumption discipline: the supervisor stays passive between watcher events and never polls `axi status` or reads panes on a timer; probe only on a decision-relevant event (one targeted read for exact gate text), before merge approval, on loop-limit/failure investigation, or on user request, and interrupt a pane only to deliver a decision, approval, or instruction.
...
- `impact_slides/renderer_v3/AGENTS.md` — schema-v1 canonical rendering kernel (typed validation + plan freeze + table compositions/format registry + line-chart tracer + grouped/horizontal/stacked bar charts + explicit arithmetic waterfalls + chart support surfaces + native heatmaps + linear/grouping + relationship/decision compositions + offline legacy→v1 migrator + generated JSON Schema + deterministic artifact publication); remaining chart/card families in later tickets
