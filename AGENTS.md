...
- Follow `docs/agents/delegated-delivery.md` for the canonical scripted PEW → run-owned worktree → visible Herdr implementer + one-shot host watcher → no-mistakes → host verification → approval-gated cheap merger workflow; new delegated branches must use `pi-extensible-workflows/<RunId>/issue-<N>` so deterministic cleanup can prove ownership. Do not substitute manual branches or tab launches.
