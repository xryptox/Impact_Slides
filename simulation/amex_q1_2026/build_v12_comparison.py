"""v12 renderer_v3 Amex full-deck comparison (observation only).
...
from simulation_probe import ProbeError, wait_for_paint_ready_charts  # noqa: E402
...
def rasterize_pdf(layouts: dict[int, str]) -> list[dict]:
    ...
    for i in range(44):
        sn = i + 1
        page = doc[i]
        # Scale page rect to exactly 1920x1080 (may letterbox if aspect differs)
        rect = page.rect
        zoom_x = W / rect.width
        zoom_y = H / rect.height
        # Independent axes → exact target size (objective requires exactly 1920x1080)
        mat = fitz.Matrix(zoom_x, zoom_y)
        pix = page.get_pixmap(matrix=mat, alpha=False)
        if pix.width != W or pix.height != H:
            # Fallback via PIL resize if matrix rounding drifts
...
def capture_html(layouts: dict[int, str]) -> tuple[list[dict], list[dict]]:
    ...
    with sync_playwright() as p:
        ...
        for sn in range(1, 45):
            ...
            try:
                ...
                img = Image.open(path)
                if img.size != (W, H):
                    # Enforce exact size if browser subpixel drift
                ...
                print(
                    f"html {sn:02d} {layout} charts={row['chart_count']} ok",
                    flush=True,
                )
                ...
                row["error"] = str(exc)
                # placeholder so SBS pipeline can still run
...
def make_contact_sheet(layouts: dict[int, str]) -> dict:
    # 8 cols x 6 rows of thumbnails from SBS (or HTML)
    cols, rows_n = 8, 6
    thumb_w, thumb_h = 480, 135  # 3840/8 : 1080 scaled
    ...
    for sn in range(1, 45):
        ...
        # show full SBS scaled into thumb
...
def main() -> int:
    ...
    html_rows, html_errors = capture_html(layouts)
    ...
    manifest = {
        ...
        "repository_commit": __import__("subprocess")
    ...
    print("capture_errors", len(html_errors))
    print("manifest", MANIFEST)
    return 1 if missing or html_errors else 0
