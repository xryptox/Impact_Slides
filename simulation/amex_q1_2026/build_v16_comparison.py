"""v16 renderer_v3 Amex full-deck comparison + DP-6 design ledger (observation only).
...
- #249 extensions: stub_ratio / support_chrome / palette / metric_floor / bar_occupancy
...
CHART_LAYOUTS = frozenset({"single_chart", "dual_chart", "chart_hero_dual"})
# Slides that need design-ledger probes even when not a chart layout.
EXTENSION_SLIDES = frozenset(
    set(DESIGN_LEDGER_STUB_RATIO_SLIDES)
    | set(DESIGN_LEDGER_SUPPORT_CHROME_SLIDES)
    | set(DESIGN_LEDGER_PALETTE_SLIDES)
    | set(DESIGN_LEDGER_METRIC_FLOOR_SLIDES)
    | set(DESIGN_LEDGER_BAR_OCCUPANCY_SLIDES)
)
...
def _ext_ok(name: str, payload: dict | None, errors: list[str]) -> dict:
    if payload is None:
        return {"ok": False, "error": next((e for e in errors if name in e), "missing")}
    return {"ok": True, **{k: v for k, v in payload.items() if k not in ("slide_number", "layout")}}


def collect_design_ledger(page, sn: int, layout: str) -> dict:
    """DP-6 + #249 design ledger. ProbeError → ok:false, never invent green."""
    is_chart = layout in CHART_LAYOUTS
    needs_work = is_chart or sn in EXTENSION_SLIDES
    if not needs_work:
        return {
            "slide_number": sn,
            "layout": layout,
    ...
    if is_chart:
        ...
        if sn in DESIGN_LEDGER_FURNITURE and not furniture_rows:
            ...
            ok_furniture = all(r.get("ok") for r in furniture_rows)
    else:
        ok_furniture = True

    # --- #249 DP-6 extensions ---
    if sn in DESIGN_LEDGER_STUB_RATIO_SLIDES:
        try:
            stub = measured_stub_ratio(page, sn, layout)
            extensions["stub_ratio"] = {
                "ok": True,
                "max_stub_share": stub.get("max_stub_share"),
                "max_share": stub.get("max_share"),
                "table_count": stub.get("table_count"),
                "tables": stub.get("tables"),
            }
        except ProbeError as exc:
            errors.append(f"stub_ratio: {exc}")
            extensions["stub_ratio"] = {"ok": False, "error": str(exc)}

    if sn in DESIGN_LEDGER_SUPPORT_CHROME_SLIDES:
        try:
            chrome = measured_support_chrome(page, sn, layout)
            extensions["support_chrome"] = {
                "ok": True,
                "head_count": chrome.get("head_count")
                if chrome.get("head_count") is not None
                else len(chrome.get("heads") or []),
                "body_count": chrome.get("body_count")
                if chrome.get("body_count") is not None
                else len(chrome.get("bodies") or []),
                "heads": chrome.get("heads"),
                "bodies": chrome.get("bodies"),
            }
        except ProbeError as exc:
            errors.append(f"support_chrome: {exc}")
            extensions["support_chrome"] = {"ok": False, "error": str(exc)}

    if sn in DESIGN_LEDGER_PALETTE_SLIDES:
        try:
            req_sky = bool(DESIGN_LEDGER_PALETTE_SLIDES[sn].get("require_sky_blue"))
            pal = measured_series_palette(page, sn, layout, require_sky_blue=req_sky)
            extensions["palette"] = {
                "ok": True,
                "colors": pal.get("colors"),
                "has_sky_blue": pal.get("has_sky_blue"),
                "require_sky_blue": req_sky,
                "series": pal.get("series"),
            }
        except ProbeError as exc:
            errors.append(f"palette: {exc}")
            extensions["palette"] = {"ok": False, "error": str(exc)}

    if sn in DESIGN_LEDGER_METRIC_FLOOR_SLIDES:
        try:
            met = measured_metric_value_styles(page, sn, layout)
            extensions["metric_floor"] = {
                "ok": True,
                "min_font_size_px": met.get("min_font_size_px"),
                "value_count": met.get("value_count"),
                "values": met.get("values"),
            }
        except ProbeError as exc:
            errors.append(f"metric_floor: {exc}")
            extensions["metric_floor"] = {"ok": False, "error": str(exc)}

    if sn in DESIGN_LEDGER_BAR_OCCUPANCY_SLIDES:
        try:
            bar = measured_bar_occupancy(page, sn, layout)
            extensions["bar_occupancy"] = {
                "ok": True,
                "charts": bar.get("charts"),
                "min_ratio": bar.get("min_ratio"),
                "min_observed_ratio": bar.get("min_observed_ratio")
                or bar.get("min_ratio_observed"),
            }
            # Keep raw probe fields if present
            for k in ("ok", "charts", "min_ratio"):
                if k in bar and k not in extensions["bar_occupancy"]:
                    extensions["bar_occupancy"][k] = bar[k]
            # Prefer any observed-ratio key the probe returns
            for k, v in bar.items():
                if "ratio" in k and k not in extensions["bar_occupancy"]:
                    extensions["bar_occupancy"][k] = v
        except ProbeError as exc:
            errors.append(f"bar_occupancy: {exc}")
            extensions["bar_occupancy"] = {"ok": False, "error": str(exc)}

    ext_ok = all(v.get("ok") for v in extensions.values()) if extensions else True
    ...
    if is_chart:
        ...
        # Non-chart extension targets (stub tables etc.)
        ok = ext_ok and not errors
    ...
    # Flatten extension keys onto the ledger object for manifest convenience
...
def capture_html(layouts: dict[int, str]) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    with sync_playwright() as p:
        ...
        for sn in range(1, 45):
            ...
            try:
                # Prior isolate sets siblings display:none; restore before probes
                # so getBoundingClientRect geometry is nonzero (stub/chrome/etc).
                page.evaluate(
                    """() => {
                ...
                if img.size != (W, H):
                    ...
                    img.save(path, "PNG")
                ledger_matters = layout in CHART_LAYOUTS or sn in EXTENSION_SLIDES
                row.update(
                    {
                        "status": (
                            "ok"
                            if dled.get("ok", True) or not ledger_matters
                            else "design_ledger_fail"
                        ),
                        "width": W,
                        "height": H,
                        "sha256": _sha256(path),
                ...
                if not dled.get("ok", True) and ledger_matters:
                    errors.append(
                        {
                            "slide_number": sn,
                            "layout_type": layout,
                ...
                print(
                    ...
                    f"ext={list((dled.get('extensions') or {}).keys())}",
                    flush=True,
                )
...
def main() -> int:
    ...
    # Unique HTML PNG hashes (lesson 32 blank-cover class)
    html_hashes = sorted({r["sha256"] for r in html_rows if r.get("sha256")})
    ...
    manifest = {
        ...
        "html_png_unique_sha256_count": len(html_hashes),
        "artifact_hashes": {
        ...
        "dp6_extension_targets": {
            "stub_ratio": list(DESIGN_LEDGER_STUB_RATIO_SLIDES),
            "support_chrome": list(DESIGN_LEDGER_SUPPORT_CHROME_SLIDES),
            "palette": {str(k): v for k, v in DESIGN_LEDGER_PALETTE_SLIDES.items()},
            "metric_floor": list(DESIGN_LEDGER_METRIC_FLOOR_SLIDES),
            "bar_occupancy": list(DESIGN_LEDGER_BAR_OCCUPANCY_SLIDES),
        },
    }
    ...
    print("html unique hashes", len(html_hashes))
